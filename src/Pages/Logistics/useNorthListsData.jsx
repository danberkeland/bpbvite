...
const summaryAttributes = ["locNick", "prodNick", "delivDate", "qty", "route"];
 ...
 * @returns
 */
const useNorthListData = ({ reportDT }) => {
  ...
  const { data: T0Orders } = useCombinedRoutedOrdersByDate({
    shouldFetch: true,
    useHolding: false,
    delivDT: reportDT.plus({ days: 0 }),
  });
  const { data: T1Orders } = useCombinedRoutedOrdersByDate({
    shouldFetch: true,
    useHolding: true,
  ...
  const calcCroixNorth = () => {
    ...
    const T0NorthFrozenOrders = T0Orders.filter(
      (order) =>
        1 &&
    ...
    const T1NorthBakedOrders = T1Orders.filter(
      (order) =>
        1 &&
    ...
    // **** 3. Baked orders ****
    const T0BakedOrders = T0Orders.filter(
      (order) =>
        1 &&
        products[order.prodNick].bakedWhere.length > 1 &&
        products[order.prodNick].packGroup === "baked pastries" &&
    ...
    const southSetoutRecord = IQB.find(
      (item) => item.id === T1Iso + "PradosetoutTime"
    );
    ...
    const retailOrdersAfterDeadline = T0Orders.filter(
      (order) =>
        1 &&
        order.isWhole === false &&
    ...
    for (let row of croixRows) {
      // **** Frozen Column ****
      const freezerNorth =
        sortedProducts.find((P) => P.forBake === row.forBake)?.freezerNorth ??
        0;

      const frozenOrderItems = T0NorthFrozenOrders.filter(
        (order) => prodNickToShapeTypeMap[order.prodNick] === row.forBake
      ...
      const bakedOrderItems = T1NorthBakedOrders.filter(
        (order) => prodNickToShapeTypeMap[order.prodNick] === row.forBake
      )
        .map((order) => order)
        .map((order) => objProject(order, summaryAttributes));
      ...
      // **** Baked Column ****
      const backporchOrders = T0BakedOrders.filter(
        (order) =>
          1 &&
      ...
      const bpbextrasOrders = T0BakedOrders.filter(
        (order) =>
          1 &&
      ...
      const afterDeadlineOrders = retailOrdersAfterDeadline
        .filter(
          (order) => prodNickToShapeTypeMap[order.prodNick] === row.forBake
        )
        .map((order) => objProject(order, summaryAttributes));
  ...
  const calcShelfProds = () => {
    if (!LOC || !PRD || !T0Orders)
      return { pivotTable: [], columnKeys: [], flatTable: [] };
    ...
    console.log(
      "FICELLE ORDERS:",
      T0Orders.filter((order) => order.prodNick === "fic")
    );

    const shelfProdOrders = T0Orders.filter(
      (order) =>
        1 &&
        products[order.prodNick].bakedWhere.length === 1 &&
        ...
        return {
          ...
          rowProps: {
            ...
            locNameShort:
              (driver === "Long Driver" ? "" : "* ") +
              (locName.length > 10
                ? locName.substring(0, 15) + "..."
                : locName), //+ truncate(row.customer, { length: 18 })
          },
        };
      });
  ...
  const calcAMNorthPradoPack = () => {
    ...
    const AMNorthOrders = T0Orders.filter(
      (order) =>
        1 &&
        ...
        order.isWhole &&
        order.qty !== 0
    ).sort(compareBy((order) => locations[order.locNick].delivOrder));
  ...
  return {
    ...
    useCalcCroixNorth: () =>
      useMemo(calcCroixNorth, [LOC, PRD, IQB, T0Orders, T1Orders]),
    useCalcShelfProds: () => useMemo(calcShelfProds, [LOC, PRD, T0Orders]),
    useAMNorthPradoPack: () =>
      useMemo(calcAMNorthPradoPack, [LOC, PRD, T0Orders]),
  };
};
